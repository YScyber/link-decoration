...
`border-bottom`一括指定プロパティを使用する際は以下のように記述してデフォルトの装飾がされないようにします。
...
`nav`要素内に5つの`a`要素があり、それぞれにリンク先が記述されています。
...
nav要素にgridプロパティを指定して1行になるように配置します。また、5つのa要素に対しnav要素全体の幅の中で`0.625%`ずつ間隔を空けます。
...
nav要素内にあるa要素に対して`border-bottom`一括プロパティと同様に以下のように記述してデフォルトの装飾がされないようにします。
...
`border`一括指定プロパティでnav要素内にある5つのa要素を黒色の直線で囲み、テキストが真ん中にくるように指定します。
